...
def symbol_to_path(path, symbol, base_dir=None, feature=False):
    """Return CSV file path given ticker symbol."""
    if base_dir is None:
        if feature:
            base_dir = os.environ.get("FEATURES_DATA_DIR",'../Data/{}/'.format(path))
        else:
...
def get_data(symbol, start_date=None, end_date=None):
    ...
    df = pd.read_csv(symbol_to_path(symbol), index_col='Datetime',
...
def get_feature_file(path, symbol, start_date=None, end_date=None):

    df = pd.read_csv(symbol_to_path(path, symbol, feature=True), index_col='Datetime',
        parse_dates=True, na_values=['nan'])
    ...
    for key in keys:
        ...
        for symbol in symbols:
            df_temp = pd.read_csv(symbol_to_path(symbol), index_col="Datetime",
                    parse_dates=True, usecols=["Datetime", key], na_values=["nan"])
...
def create_df_benchmark(symbol, start_date, end_date, num_shares):
    ...
    benchmark_prices = get_data(symbol, start_date, end_date)['Close']
    ...
    df_benchmark_trades = pd.DataFrame(
        ...
        columns=["Datetime", "Shares"])
    df_benchmark_trades.set_index("Datetime", inplace=True)
    return df_benchmark_trades

def create_df_trades(orders, num_shares=100, cash_posi=0):
    ...
    non_cash_orders = orders[orders != cash_posi]
    trades = []
    for date in non_cash_orders.index:
        
        trades.append((date, non_cash_orders.loc[date]*num_shares))
        
    df_trades = pd.DataFrame(trades, columns=["Datetime", "Shares"])
...
def thi_to_decimal(trinary_array):
    """Convert a trinary array to decimal"""
    return int(''.join(str(x) for x in trinary_array), 3)